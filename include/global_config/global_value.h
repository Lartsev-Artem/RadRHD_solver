...
#define STRUCT_FIELS_T \
/*--------------------------Файлы сеток----------------------------------------*/ \
    X(std::string, name_file_vtk) \
    X(std::string, name_file_sphere_direction) \
/*--------------------------Глобальрные адреса----------------------------------------*/ \
    X(std::string, base_adress)\
	X(std::string, illum_geo_adress)\
	X(std::string, graph_adress)\
	X(std::string, solve_adress)\
/*--------------------------Настроичные файлы----------------------------------------*/ \
...
#define X(type, name) {#name, p.name},
	j = nlohmann::json{ STRUCT_FIELS_T };
#undef X	
}
