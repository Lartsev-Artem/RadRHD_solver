 ...
 * @note Файл должен содержать в первой строке число элементов. Далее последовательные данные.
 * @warning размер типа данных должен соответствовать данным в файле
 * @return size_t - ::e_type_completion
 */
template<typename T> size_t ReadSimple(const std::string& name_file, std::vector<T>& data) 
{
	FILE* f;
 ...
 * @return size_t - ::e_type_completion
 */
template<typename geo_elem> size_t ReadGridGeo(const std::string& name_file, std::vector<geo_elem>& data) 
{ 
	FILE* f;
	OPEN_FILE(f, name_file.c_str(), "rb");
 ...
 * @param[in] is_print флаг печати информации о считанных файлах (default=false).
