...
//! Не использовать такую структура чтения!. resize cells не нужен!!!. При излучении данные распределны по массивам?
	...
	std::vector<Type>& vec_res_bound)
{
	...
	if (ReadSimple(gbl_files.name_file_res, vec_res_bound)) return e_completion_fail;
	...
	{
		if (ReadSimple(gbl_files.name_file_x0_loc + std::to_string(disp[myid]+i) + ".bin", vec_x0[i])) return e_completion_fail;
		if (ReadSimple(gbl_files.graph_address + std::to_string(disp[myid]+i) + ".bin", sorted_id_cell[i])) return e_completion_fail;
		if (ReadSimple(gbl_files.name_file_state_face + std::to_string(disp[myid]+i) + ".bin", face_states[i])) return e_completion_fail;
	}
