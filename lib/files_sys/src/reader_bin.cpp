...
int files_sys::bin::ReadRadiationFaceTrace(const int count_dir, const global_files_t &gbl_files,
  ...
  for (int i = 0; i < send[myid]; i++) {
    ...
    if (ReadSimple(gbl_files.graph_address + F_GRAPH_BOUND_FACE + std::to_string(disp[myid] + i) + ".bin", boundary_faces.sorted_id_bound_face[i]))
      return e_completion_fail;
    ...
    if (ReadSimple(gbl_files.illum_geo_address + F_DIRECTION_REFLECTION + std::to_string(disp[myid] + i) + ".bin", boundary_faces.reflection_dir_id[i]))
      return e_completion_fail;

    if (ReadSimple(gbl_files.illum_geo_address + F_FACES_REFLECTION + std::to_string(disp[myid] + i) + ".bin", boundary_faces.reflection_faces_id[i]))
      return e_completion_fail;
#endif
  }
