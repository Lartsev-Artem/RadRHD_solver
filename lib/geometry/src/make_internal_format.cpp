  ...
  for (size_t num_cell = 0; num_cell < n; num_cell++)
  {
    ...
    for (size_t num_face = 0; num_face < CELL_SIZE; num_face++)
    {
      ...
      if (neighbors[id] < 0) // граница
      {
        ...
        {
          neighbors[id] = e_bound_outer_surface; // свободная поверхность
        }
        else
        {
...
#error "Need rebuild GetNeighborFace2D (CELL_SIZE->3)"
...
int BuildSphereDirection(const std::string &sphere_vtk_grid, const std::string &out_address, bool extended)
{
  ...
  if (files_sys::bin::WriteSimple(out_address + F_SURFACE_SPHERE_DIRECTION, faces))
    return e_completion_fail;

  if (extended)
  {
    std::vector<IntId> neighbors;
    GetNeighborFace2D(grid, neighbors);

    std::vector<IntId> nodes_id;
    GetCellsPointsIdSurface(grid, nodes_id);

    interpolation_mesh_direction_t mesh;
    GetGridStructSurface(grid, mesh);

    if (files_sys::bin::WriteSimple(out_address + F_NEIGHBOR_SURFACE, neighbors))
      return e_completion_fail;

    if (files_sys::bin::WriteSimple(out_address + F_NODES_ID_SURFACE, nodes_id))
      return e_completion_fail;

    if (files_sys::bin::WriteSimple(out_address + F_NUM_NEIGHS_SURFACE, mesh.num_of_neighs))
      return e_completion_fail;

    if (files_sys::bin::WriteSimple(out_address + F_NODE_CELLS_ID_SURFACE, mesh.node_cells_id))
      return e_completion_fail;

    if (files_sys::bin::WriteSimple(out_address + F_NODES_COORD_SURFACE, mesh.nodes_coord))
      return e_completion_fail;
  }
