...
#define WRITE_POS(_file, _msg){ \
    ...
    out << _msg << " :: " << __FILE__ << " " << __FUNCTION__ << ", " <<__LINE__<<"c.\n"; \
    out.close();\
}
...
#define EXIT_ERR(...){WRITE_LOG_ERR(__VA_ARGS__); D_LD;}
#define RETURN_ERR(...) {WRITE_LOG_ERR(__VA_ARGS__) return e_completion_fail; }
...
std::ofstream ofile; \
ofile.open(Files_log + std::to_string(get_mpi_id())+".txt", std::ios::app); \
ofile << str; \
ofile.close(); }

#define DBG_POINT WRITE_LOG("dbg_p: "<< __FILE__ << " " << __FUNCTION__ << ", " <<__LINE__<<"c.\n")
