...
const Type sigma_thomson =
    6.652 * 1e-29;  //сечение томсоновского рассеяния [m^2]
...
//--------------------------Файлы расчётных данных(отдельные файлы по
//направлениям)-----------------------------------//
...
//--------------------------Файлы трассировки сквозь внутреннюю
//границу----------------------------------------//
...
#undef X
  ...
  global_files_t(file_name_t BASE_ADDRESS = "",
                 file_name_t address_illum_geo_file = "") {
  ...
  void print() {
    ...
    while (str <
           (std::string*)this + sizeof(global_files_t) / sizeof(std::string)) {
      std::cout << *str++ << '\n';
    }
  }
#endif
