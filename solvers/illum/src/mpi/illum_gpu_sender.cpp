...
static void ReCalcLocalIllum(const grid_directions_t &dir_grid, grid_t &grid) {
  ...
  section_2.size = size_second_section;
  {
    ...
    for (int src = 0; src < np; src++) {
      ...
      for (int j = size_first_section; j < send_count[src]; j++) {
        int tag = disp[src] + j;
        MPI_Recv_init(grid.Illum + tag, (int)size_msg, MPI_RECV_ILLUM_T, src, tag, MPI_COMM_ILLUM, &section_2.requests_rcv[cc++ /*tag - local_size*/]);
      }
    }
