...
Type illum::scattering::GetIntScatteringByDirection(const IdType num_cell, const Vector3 &direction, const IdType size_one_direction, const Type *illum_old,
                                                    const grid_directions_t &grid_direction) {

  // const int size_one_direction = illum_old.size() / N_dir;
  Type S = 0;
  for (IdType num_direction = 0; num_direction < grid_direction.size; num_direction++) {
