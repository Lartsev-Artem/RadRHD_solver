...
int illum::cpu::CalculateIllumFace(const grid_directions_t &grid_direction,
                                   ...
                                   const std::vector<std::vector<cell_local>> &vec_x0, std::vector<face_node_points> &vec_x,
...
#pragma omp parallel default(none) firstprivate(count_directions) shared(sorted_id_face, sorted_id_bound_face, inner_bound_code, vec_x0, vec_x, grid, norm)
    {
...
#pragma error "need recalc code bound from cell to face"
        ...
        for (auto fc_pair : sorted_graph[num_direction]) {
          ...
          elem_t *cell = &grid.cells[fc_pair.cell];
...
#endif
            const Vector3 &x = vec_x[fc_pair.face].x[num_node];

            I[num_node] = GetIllum(x, X0_ptr->s, I_x0, grid.scattering[num_direction * count_cells + fc_pair.cell], *cell);
            X0_ptr++;
          } // num_node

          //записываем значение коэффициентов на грани
          (*inter_coef)[fc_pair.face] = (I[0] + I[1] + I[2]) / 3.;
        }
