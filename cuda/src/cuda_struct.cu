...
static int grid_init_device(grid_device_t *&grid_device, const int num_cell, const int num_cell_loc, const int cell_loc_shift, const int num_dir, const int start, const int end) {
  ...
  CUDA_INIT_GRID_PTR_DEVICE(grid_device->illum, device_host_ptr.illum, (CELL_SIZE * num_dir * num_cell * sizeof(Type)));

  CUDA_INIT_GRID_PTR_DEVICE(grid_device->int_scattering, device_host_ptr.int_scattering, loc_size * num_cell * sizeof(Type));

  CUDA_MEMCPY_TO_DEVICE(&grid_device->loc_size, &num_cell_loc, sizeof(num_cell_loc));
  CUDA_MEMCPY_TO_DEVICE(&grid_device->shift, &cell_loc_shift, sizeof(cell_loc_shift));

  CUDA_INIT_GRID_PTR_DEVICE(grid_device->energy, device_host_ptr.energy, num_cell_loc * sizeof(Type));

  CUDA_INIT_GRID_PTR_DEVICE(grid_device->divstream, device_host_ptr.divstream, num_cell_loc * sizeof(Type));
  CUDA_INIT_GRID_PTR_DEVICE(grid_device->divimpuls, device_host_ptr.divimpuls, num_cell_loc * sizeof(cuda_vector_t<Type, 3>));

  CUDA_INIT_GRID_PTR_DEVICE(grid_device->volume, device_host_ptr.volume, num_cell * sizeof(Type));
  CUDA_INIT_GRID_PTR_DEVICE(grid_device->areas, device_host_ptr.areas, CELL_SIZE * num_cell * sizeof(Type));
  CUDA_INIT_GRID_PTR_DEVICE(grid_device->normals, device_host_ptr.normals, CELL_SIZE * num_cell * sizeof(cuda_vector_t<Type, 3>));

#ifdef CUDA_FULL_ARRAYS
  CUDA_INIT_GRID_PTR_DEVICE(grid_device->energy, device_host_ptr.energy, num_cell_loc * sizeof(Type));
  CUDA_INIT_GRID_PTR_DEVICE(grid_device->stream, device_host_ptr.stream, num_cell_loc * sizeof(cuda_vector_t<Type, 3>));
  CUDA_INIT_GRID_PTR_DEVICE(grid_device->impuls, device_host_ptr.impuls, num_cell_loc * sizeof(cuda_vector_t<Type, 9>));
#endif
...
static int grid_init_copy_host_to_device(const std::vector<Type> &host_volume, const std::vector<Type> &host_areas, const std::vector<Normals> &host_normals,
  ...
  std::vector<cuda_vector_t<Type, 3>> dev_norm(host_normals.size() * CELL_SIZE);
  for (size_t i = 0; i < host_normals.size(); i++) {
    for (size_t j = 0; j < CELL_SIZE; j++) {
      dev_norm[i * CELL_SIZE + j].data[0] = host_normals[i].n[j][0];
      dev_norm[i * CELL_SIZE + j].data[1] = host_normals[i].n[j][1];
      dev_norm[i * CELL_SIZE + j].data[2] = host_normals[i].n[j][2];
    }
  }
  ...
  CUDA_MEMCPY_TO_DEVICE(device_host_ptr.areas, host_areas.data(), CELL_SIZE * N * sizeof(host_areas[0]));
...
static int InitDevice(grid_directions_device_t *&grid_dir_device, grid_device_t *&grid_device,
  ...
  if (ReadNormalFile(name_file_normals, normals))
    RETURN_ERR("Error reading file normals\n");
  if (ReadSimpleFileBin(name_file_squares, squares_faces))
    RETURN_ERR("Error reading file squares_faces\n");
  if (ReadSimpleFileBin(name_file_volume, volume))
    RETURN_ERR("Error reading file volume\n");

  grid_directions_init_copy_host_to_device(grid_dir_device, grid_dir_host);

  grid_init_device(grid_device, grid_host.size, grid_host.loc_size, grid_host.loc_shift, grid_dir_host.size, start, end);

  CUDA_MALLOC_HOST(&grid_host.Illum, (CELL_SIZE * grid_dir_host.size * grid_host.size * sizeof(Type)));
  ...
  {
    CUDA_MALLOC_HOST(&grid_host.divstream, (grid_host.loc_size * sizeof(Type)));
    CUDA_MALLOC_HOST(&grid_host.divimpuls, (grid_host.loc_size * sizeof(Vector3)));

#ifdef ON_FULL_ILLUM_ARRAYS
    CUDA_MALLOC_HOST(&grid_host.energy, (grid_host.loc_size * sizeof(Type)));
    CUDA_MALLOC_HOST(&grid_host.stream, (grid_host.loc_size * sizeof(Vector3)));
    CUDA_MALLOC_HOST(&grid_host.impuls, (grid_host.loc_size * sizeof(Matrix3)));
#endif
