...
static cudaStream_t cuda_streams[cuda::e_сuda_count];

static cudaEvent_t cuda_events[cuda::e_сuda_count * 3];

void CudaEventPrint() {
  for (int i = 0; i < cuda::e_сuda_count * 3; i++) {
    // synchronize
...
void SetDevice(const int num_dev) {
  ...
  CUDA_CALL_FUNC(cudaStreamCreateWithPriority, &cuda_streams[eCuda_scattering_1], cudaStreamNonBlocking, priority_high);
  CUDA_CALL_FUNC(cudaStreamCreateWithPriority, &cuda_streams[eCuda_scattering_2], cudaStreamNonBlocking, (priority_high + priority_low) / 2);
  CUDA_CALL_FUNC(cudaStreamCreateWithPriority, &cuda_streams[eCuda_params], cudaStreamNonBlocking, priority_low);
}
...
int CalculateIntScatteringAsyncMPIStream(const grid_directions_t &grid_dir, grid_t &grid,
  ...
  CUDA_MEMCPY_TO_HOST_ASYNC_STREAM(grid.scattering + start * N, device_host_ptr.int_scattering + start * N, N * (end - start) * sizeof(grid.scattering[0]),
                                   cuda_streams[stream]);
...
int CalculateIntScattering(const grid_directions_t &grid_dir, grid_t &grid) {
  ...
  CUDA_MEMCPY_TO_DEVICE(device_host_ptr.illum, grid.Illum, N * M * CELL_SIZE * sizeof(grid.Illum[0]));
...
int CalculateAllParam(const grid_directions_t &grid_dir, grid_t &grid) {
  ...
  CUDA_MEMCPY_TO_HOST(grid.divstream, device_host_ptr.divstream, N_loc * sizeof(grid.divstream[0]));
  CUDA_MEMCPY_TO_HOST(grid.divimpuls, device_host_ptr.divimpuls, N_loc * sizeof(grid.divimpuls[0]));

#ifdef CUDA_FULL_ARRAYS
  CUDA_MEMCPY_TO_HOST_ASYNC(grid.energy, device_host_ptr.energy, N_loc * sizeof(grid.energy[0]));
  CUDA_MEMCPY_TO_HOST_ASYNC(grid.stream, device_host_ptr.stream, N_loc * sizeof(grid.stream[0]));
  CUDA_MEMCPY_TO_HOST_ASYNC(grid.impuls, device_host_ptr.impuls, N_loc * sizeof(grid.impuls[0]));
#endif
...
int CalculateAllParamStream(const grid_directions_t &grid_dir, grid_t &grid, e_cuda_stream_id_t st) {
  ...
  CUDA_TREADS_1D(threads, BS);
  CUDA_BLOCKS_1D(blocks, N_loc, BS);
  ...
  // не асинхронно т.к. сразу затем идет расчет газовый
  CUDA_MEMCPY_TO_HOST_ASYNC_STREAM(grid.divstream, device_host_ptr.divstream, N_loc * sizeof(grid.divstream[0]), cuda_streams[st]);
  CUDA_MEMCPY_TO_HOST_ASYNC_STREAM(grid.divimpuls, device_host_ptr.divimpuls, N_loc * sizeof(grid.divimpuls[0]), cuda_streams[st]);

#ifdef CUDA_FULL_ARRAYS
  CUDA_MEMCPY_TO_HOST_ASYNC_STREAM(grid.energy, device_host_ptr.energy, N_loc * sizeof(grid.energy[0]), cuda_streams[st]);
  CUDA_MEMCPY_TO_HOST_ASYNC_STREAM(grid.stream, device_host_ptr.stream, N_loc * sizeof(grid.stream[0]), cuda_streams[st]);
  CUDA_MEMCPY_TO_HOST_ASYNC_STREAM(grid.impuls, device_host_ptr.impuls, N_loc * sizeof(grid.impuls[0]), cuda_streams[st]);
#endif
