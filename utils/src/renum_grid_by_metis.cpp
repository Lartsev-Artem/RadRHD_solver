...
static int sort_bound_cells(const std::string &name_file_vtk, const std::vector<int> &graph, std::vector<Eigen::Vector4i> &cells)
{
  ...
  if (files_sys::vtk::Read(name_file_vtk, ugrid))
  {
  ...
  if (GetNeighborFace3D(ugrid, neighbors))
  {
  ...
  {
    size_nodes[i] = std::accumulate(graph.begin(), graph.end(), 0, [&i](int in, int a)
                                    { return in + (int)(a == i); });
  }
  ...
  for (int np = 0; np < nodes; np++)
  {
    ...
    for (int i = start; i < end; i++, neigh += CELL_SIZE, cur++)
    {
    ...
    cur = cells.begin() + start;
    neigh = neighbors.begin() + start * CELL_SIZE;
    for (int i = start; i < end; i++, neigh += CELL_SIZE, cur++)
    {
      for (int k = 0; k < CELL_SIZE; k++)
      {
        ...
        {
          int neigh_np = graph[neigh[k] / CELL_SIZE];
          if (neigh_np > np) // соседний узел справа
          {
...
int FUNC_NAME(RenumNetgenByMetisToVtk)(int argc, char *argv[])
{
  ...
  std::generate(node_cells.begin(), node_cells.end(),
                [&i, &graph, &cell]
                { i++; return std::make_pair(graph[i-1], cell[i-1]); });
  ...
  std::sort(node_cells.begin(), node_cells.end(),
            [](std::pair<int, Eigen::Vector4i> l, std::pair<int, Eigen::Vector4i> r)
  ...
  std::generate(cell.begin(), cell.end(),
                [&i, &node_cells]
                { i++; return node_cells[i-1].second; });

  i = 0;
  std::generate(graph.begin(), graph.end(),
                [&i, &node_cells]
                { i++; return node_cells[i-1].first; });
